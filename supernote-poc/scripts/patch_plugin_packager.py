#!/usr/bin/env python3
"""Make the generated Supernote packager fail closed for native InkBridge."""
...
EXPECTED_PACKAGE = "com.inkbridgepoc.InkBridgeFolderPackage"
EXPECTED_SOURCE = (
    "android/app/src/main/java/com/inkbridgepoc/InkBridgeFolderPackage.kt"
)
EXPECTED_APPLICATION = (
    "android/app/src/main/java/com/inkbridgepoc/MainApplication.kt"
)
...
'''
STRICT_PACKAGE = f'''    # InkBridge requires its native folder bridge. Verify the exact generated
    # source and registration rather than trusting the packager's best-effort scan.
    ...
    if [[ ! -f "$expected_react_source" ]]; then
        write_color_output "Required InkBridge native package source is missing" "Red"
        return 1
    fi
    if [[ ! -f "$expected_application" ]] || \\
       ! grep -Fq 'add(InkBridgeFolderPackage())' "$expected_application"; then
        write_color_output "Required InkBridge native package registration is missing" "Red"
        return 1
    fi
...
STRICT_NATIVE_BUILD = '''        if ! build_android_apk "$project_root" "$gen_cfg"; then
            write_color_output "Required InkBridge native APK build failed" "Red"
            return 1
        fi
        if ! copy_apk_and_update_config "$project_root" "$gen_dir" "$gen_cfg"; then
            write_color_output "Required InkBridge native APK packaging failed" "Red"
            return 1
        fi
    else
        write_color_output "Required InkBridge native build was not selected" "Red"
        return 1
    fi
'''
...
def patch_text(text: str) -> str:
    text = replace_once(
        text, UPSTREAM_PACKAGE_SCAN, STRICT_PACKAGE, "native package scan"
    )
    text = replace_once(
        text,
...
def main() -> None:
    ...
    path.write_text(patch_text(original), encoding="utf-8", newline="\n")
    print(f"Hardened generated InkBridge native packager: {path}")
